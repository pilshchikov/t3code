...
it.layer(NodeSqliteClient.layerMemory())("049_ProjectionThreadsActiveOrderKey", (it) => {
  it.effect("migrates old threads without changing their timestamps or assigning an order", () =>
    Effect.gen(function* () {
      ...
      // Upgrade from the last installed fork, including its workspace-root migrations.
      yield* runMigrations({ toMigrationInclusive: 49 });
      const now = "2026-01-01T00:00:00.000Z";
      const roots = '[{"path":"/project"},{"path":"/second"}]';
      yield* sql`
        INSERT INTO projection_projects (
          project_id, title, workspace_root, workspace_roots_json, scripts_json, created_at, updated_at
        ) VALUES ('project-1', 'Existing project', '/project', ${roots}, '[]', ${now}, ${now})
      `;
      yield* sql`
      ...
      const projects = yield* sql<{ readonly roots: string }>`
        SELECT workspace_roots_json AS roots FROM projection_projects WHERE project_id = 'project-1'
      `;
      assert.deepEqual(projects, [{ roots }]);
      const pullRequests = yield* sql<{ readonly branchPullRequest: string | null }>`
        SELECT branch_pull_request_json AS "branchPullRequest" FROM projection_threads WHERE thread_id = 'thread-1'
      `;
      assert.deepEqual(pullRequests, [{ branchPullRequest: null }]);
