...
layer("050_ProjectionThreadPullRequests", (it) => {
  it.effect("creates the link table and backfills legacy single links", () =>
    Effect.gen(function* () {
      ...
      yield* sql`INSERT INTO project_memories VALUES ('project-1', 'hosts', 'Host locations', 'private host details', '2026-09-10T00:00:00.000Z')`;
      ...
      assert.deepStrictEqual(executed, [[53, "ProjectionThreadPullRequests"]]);
      const memories = yield* sql`SELECT content FROM project_memories WHERE name = 'hosts'`;
      assert.deepStrictEqual(memories, [{ content: "private host details" }]);
