...
it.layer(TestLayer)("GitVcsDriver core integration", (it) => {
  ...
  describe("review diff previews", () => {
    ...
    it.effect("loads only the requested review source and file", () =>
      Effect.gen(function* () {
        ...
        const selected = yield* driver.getReviewDiffPreview({
          cwd,
          sourceKind: "working-tree",
          file: { path: "selected.txt", previousPath: null, sourceKind: "working-tree" },
        });
    ...
    it.effect("lists every untracked file in the working-tree overview", () =>
      Effect.gen(function* () {
        const cwd = yield* makeTmpDir();
        yield* initRepoWithCommit(cwd);
        const driver = yield* GitVcsDriver.GitVcsDriver;
        ...
        assert.strictEqual(preview.sources[0]?.files?.length, 25);
      }),
    );
