...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect.each([
    { configDir: "./synthetic config's $literal", inherited: undefined },
    { configDir: "", inherited: ".synthetic config's $literal" },
    { configDir: "", inherited: " /synthetic/path with edge spaces " },
  ])(
    "reports the same Claude config and cwd used by the spawned query ($configDir, $inherited)",
    ({ configDir, inherited }) => {
      const harness = makeHarness({
        claudeConfig: { configDir },
      ...
      return Effect.gen(function* () {
        ...
        const expectedConfigDir = configDir ? NodePath.resolve(configDir) : inherited;
