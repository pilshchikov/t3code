...
import { UsageLimitHistory } from "./usage/UsageLimitHistory.ts";
...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const multiwork = yield* MultiworkService.MultiworkService;
      const resumableSessions = yield* ResumableSessionDiscovery;
      ...
      const usageLimitHistory = yield* UsageLimitHistory;
      ...
      const dispatchBootstrapTurnStart = (
        ...
        Effect.gen(function* () {
          ...
          const bootstrapProgram = Effect.gen(function* () {
            ...
            if (prepareWorktree && prepareWorktree.mode !== "multiwork" && shouldPrepareWorktree) {
            ...
            if (bootstrap?.prepareWorktree?.mode === "multiwork") {
              ...
              yield* worktreeSetupTracker.stageStatus(
                ...
                "checkout",
                "done",
                copy.reused ? "Existing copy" : "Multiwork copy ready",
              );
            ...
            Effect.catchCause((cause) => {
              ...
              if (Cause.hasInterruptsOnly(cause)) {
                ...
                const removeCreatedWorktree =
                  ...
                  bootstrap.prepareWorktree.mode !== "multiwork"
      ...
      return WsRpcGroup.of({
        ...
        [WS_METHODS.sessionsListResumable]: (input) =>
          observeRpcEffect(
            WS_METHODS.sessionsListResumable,
            resumableSessions.list({ cwd: input.cwd }),
            { "rpc.aggregate": "workspace" },
          ),
        [WS_METHODS.projectsSearchCode]: (input) =>
          observeRpcEffect(
            WS_METHODS.projectsSearchCode,
            workspaceEntries.searchCode(input).pipe(
              Effect.mapError(
                (cause) =>
                  new ProjectSearchCodeError({
                    message: `Failed to search workspace code: ${cause.message}`,
                    cause,
                  }),
              ),
            ),
        ...
        [WS_METHODS.vcsPull]: (input) =>
          observeRpcEffect(
            WS_METHODS.vcsPull,
            gitWorkflow.pullCurrentBranch(input.cwd, input.refName).pipe(
              Effect.matchCauseEffect({
        ...
        [WS_METHODS.vcsFetch]: (input) =>
          observeRpcEffect(
            WS_METHODS.vcsFetch,
            gitWorkflow
              .fetchCurrentBranch(input.cwd, ...(input.scope ? [{ scope: input.scope }] : []))
              .pipe(Effect.tap(() => refreshGitStatus(input.cwd))),
            { "rpc.aggregate": "git" },
          ),
