...
const makeHarness = Effect.fn("makeThreadSettlementHarness")(function* (options: HarnessOptions) {
  ...
  const initialSettings: ServerSettings = options.settings ?? {
    ...DEFAULT_SERVER_SETTINGS,
    sidebarAutoSettleMode: "inactivity",
  };
  const settings = yield* Ref.make(initialSettings);
...
describe("ThreadSettlementReactor", () => {
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          yield* reactor.drain;
          // The branch without a PR settles; the linked open PR remains protected.
          assert.deepStrictEqual(
  ...
  it.effect(
    "settles inactivity without settling recent closed PRs and skips protected threads",
    () =>
      Effect.scoped(
        Effect.gen(function* () {
          yield* TestClock.setTime(Date.parse(NOW));
  ...
  it.effect("reevaluates inactivity once per minute without changing policies", () =>
    Effect.scoped(
      Effect.gen(function* () {
        yield* TestClock.setTime(Date.parse(NOW));
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          yield* fixture.updateSettings({ sidebarAutoSettleMode: "never" });
  ...
  it.effect("protects inactive threads with an open pull request", () =>
    Effect.scoped(
      Effect.gen(function* () {
        yield* TestClock.setTime(Date.parse(NOW));
        const fixture = yield* makeHarness({
          snapshot: makeSnapshot([
            makeThread("inactive-open-pr", {
              linkedPullRequest: {
                projectId: PROJECT_ID,
                repository: "owner/repository",
                number: 42,
  ...
  it.effect("keeps inactive PR-bearing threads active when the host is unavailable", () =>
    Effect.scoped(
      Effect.gen(function* () {
        yield* TestClock.setTime(Date.parse(NOW));
        const fixture = yield* makeHarness({
  ...
  it.effect("waits for the shared PR lookup before settling inactive linked threads", () =>
    Effect.scoped(
      Effect.gen(function* () {
        yield* TestClock.setTime(Date.parse(NOW));
        ...
        yield* Effect.gen(function* () {
          ...
          yield* reactor.drain;
          assert.strictEqual((yield* Ref.get(fixture.commands)).length, 0);
          assert.strictEqual((yield* Ref.get(fixture.summaryCalls)).length, 1);
        }).pipe(Effect.provide(fixture.layer));
      }),
    ),
