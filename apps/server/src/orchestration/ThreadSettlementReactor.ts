...
export const make = Effect.gen(function* () {
  ...
  const sweep = Effect.fn("ThreadSettlementReactor.sweep")(function* (
    ...
    // Threads without a branch or saved PR can settle without network access.
    // The fork protects open PRs even in inactivity mode, so PR-bearing
    // threads must wait until lookup confirms there is no open PR.
