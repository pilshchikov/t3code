...
describe("shouldAutoSettleThread", () => {
  ...
  it("keeps automatic settlement disabled by the fork default", () => {
    ...
    expect(decide(makeThread(), { state: "merged", mergedAt: NOW }, { mode: "never" })).toBe(false);
  });

  it("settles only finished pull requests in change-request mode", () => {
    expect(
      decide(makeThread(), { state: "merged", mergedAt: NOW }, { mode: "change-request" }),
    ).toBe(true);
    expect(decide(makeThread(), null, { mode: "change-request" })).toBe(false);
  });

  it("uses only inactivity in inactivity mode", () => {
    expect(decide(makeThread(), null, { mode: "inactivity" })).toBe(true);
    expect(
      decide(
        makeThread({ latestUserMessageAt: "2026-08-28T11:00:00.000Z" }),
