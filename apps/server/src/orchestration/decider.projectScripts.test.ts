...
it.layer(NodeServices.layer)("decider project scripts", (it) => {
  ...
  it.effect("allows project.create to reuse an active workspace root", () =>
    Effect.gen(function* () {
      const now = "2026-01-01T00:00:00.000Z";
      const initial = createEmptyReadModel(now);
      const readModel = yield* projectEvent(initial, {
      ...
      const event = Array.isArray(result) ? result[0] : result;
      expect(event.type).toBe("project.created");
      expect(event.payload.projectId).toBe(asProjectId("project-duplicate-root"));
    }),
  );

  it.effect("allows project.meta.update to reuse another active workspace root", () =>
    Effect.gen(function* () {
      const now = "2026-01-01T00:00:00.000Z";
      const initial = createEmptyReadModel(now);
      ...
      const event = Array.isArray(result) ? result[0] : result;
      expect(event.type).toBe("project.meta-updated");
      expect(event.payload.projectId).toBe(asProjectId("project-second"));
    }),
  );
