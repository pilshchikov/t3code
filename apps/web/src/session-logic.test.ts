...
describe("deriveTimelineEntries", () => {
  ...
  it("preserves fork turn plans across streaming, appending, editing, and removal", () => {
    const plan = {
      ...
      plan: {
        createdAt: streamingMessage.createdAt,
        turnId: streamingMessage.turnId,
        steps: [{ step: "Check files", status: "inProgress" as const }],
      },
    };
    const first = deriveTimelineEntriesWithState([streamingMessage], [], [], null, [plan]);
    expect(first.entries).toEqual(deriveTimelineEntries([streamingMessage], [], [], [plan]));
    const message = { ...streamingMessage, text: "Checking" };
    const streaming = deriveTimelineEntriesWithState([message], [], [], first, [plan]);
    expect(streaming.entries[1]).toBe(first.entries[1]);
    const secondPlan = { ...plan, id: "turn-plan:second" };
    const appended = deriveTimelineEntriesWithState([message], [], [], streaming, [
      plan,
      secondPlan,
    ]);
    expect(appended.entries).toEqual(deriveTimelineEntries([message], [], [], [plan, secondPlan]));
    const done = {
      ...plan,
      plan: { ...plan.plan, steps: [{ step: "Check files", status: "completed" as const }] },
    };
    const edited = deriveTimelineEntriesWithState([message], [], [], appended, [done, secondPlan]);
    expect(edited.entries).toEqual(deriveTimelineEntries([message], [], [], [done, secondPlan]));
    const removed = deriveTimelineEntriesWithState([message], [], [], edited, []);
    expect(removed.entries).toEqual(deriveTimelineEntries([message], [], [], []));
  });
