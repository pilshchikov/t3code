...
import { CollapsibleChatImage } from "./chat/CollapsibleChatImage";
import {
...
function ChatMarkdownImage(props: ChatMarkdownImageProps) {
  if (!props.standalone) return <ChatMarkdownImageContent {...props} />;
  const imageKey = JSON.stringify(
    props.actionsSource.asset ??
      props.actionsSource.reference ??
      props.originalUrl ??
      props.src ??
      props.copyMarkdown,
  );
  return (
    <CollapsibleChatImage imageKey={`markdown:${imageKey}`} label={props.alt}>
      <ChatMarkdownImageContent {...props} />
    </CollapsibleChatImage>
  );
}

function ChatMarkdownImageContent(props: ChatMarkdownImageProps) {
...
const CHAT_MARKDOWN_COMPONENTS = {
  ...
  a: function MarkdownAnchor({ node, href, children, title: _title, ...props }) {
    ...
    const fileLinkMeta = normalizedHref
      ? (markdownFileLinkMetaByHref.get(normalizedHref) ??
        resolveMarkdownFileLinkMeta(normalizedHref, cwd, workspaceRoot ?? cwd))
      : null;
    if (!fileLinkMeta) {
  ...
  code: function MarkdownCode({ node, children, className, ...props }) {
    const { cwd, workspaceRoot, inlineCodeFileLinkMetaByText, fileLinkChip } = use(
    ...
    if (node?.properties?.dataInlineCode != null) {
      ...
      const fileLinkMeta =
        ...
        resolveInlineCodeFileLinkMeta(codeText, cwd, workspaceRoot ?? cwd);
      if (fileLinkMeta) {
        return fileLinkChip(
          fileLinkMeta,
          `\`${codeText}\``,
...
function ChatMarkdown({
  ...
  return (
    <div
      ref={markdownRef}
      className={cn(
        "chat-markdown w-full min-w-0 text-[length:var(--font-size-chat-message,0.875rem)] leading-relaxed text-foreground/[calc(80%+var(--appearance-contrast-boost)/5)] [overflow-wrap:anywhere] [word-break:break-word]",
        className,
      )}
