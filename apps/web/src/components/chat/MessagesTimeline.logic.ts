...
export function deriveMessagesTimelineRows(input: {
  ...
  const workEntryIsActiveTurnActivity = (entry: WorkLogEntry) =>
    entry.toolLifecycleStatus === "inProgress" ||
  ...
  for (let index = 0; index < input.timelineEntries.length; index += 1) {
    ...
    if (timelineEntry.kind === "work") {
      ...
      if (visibleGroupedEntries.length > 0) {
        ...
        if (onlyToolEntries && activeInProgressToolEntries.length > 0) {
        ...
        } else if (onlyToolEntries && visibleGroupedEntries.length === 1) {
          ...
          nextRows.push({
            ...
            hasFailure:
              visibleGroupedEntries.findLast(workLogEntryIsToolLike) !== undefined &&
              workEntryDisplayIndicatesToolFailure(
                visibleGroupedEntries.findLast(workLogEntryIsToolLike)!,
              ),
          });
          if (expanded) {
            nextRows.push(
          ...
          if (hiddenEntries.length > 0) {
            ...
            nextRows.push({
              ...
              summary: hasDetailedFailure ? summarizeToolGroup(visibleGroupedEntries) : null,
              summaryKind: hasDetailedFailure ? toolGroupSummaryKind(visibleGroupedEntries) : null,
              hasFailure:
                latestToolEntry !== undefined &&
                workEntryDisplayIndicatesToolFailure(latestToolEntry) &&
                (hiddenEntries.some(workEntryDisplayIndicatesToolFailure) || hasDetailedFailure),
            });
          }
        }
      }
  ...
  if (input.isWorking && (input.timelineEntries.length === 0 || latestToolFailed)) {
    nextRows.push({
      kind: "thinking",
