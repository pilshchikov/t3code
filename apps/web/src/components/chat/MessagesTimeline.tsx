          ...
          <div className="mb-2 grid max-w-[420px] grid-cols-2 gap-2">
            {regularImages.map((image: ChatImageAttachment) => (
              <CollapsibleChatImage
                key={image.id}
                imageKey={`attachment:${row.message.id}:${image.id}`}
                label={image.name}
              >
                <div className="aspect-[4/3] overflow-hidden rounded-lg border border-border/80 bg-background/70">
                  {image.previewUrl ? (
                    <button
                      type="button"
...
const WorkGroupSection = memo(function WorkGroupSection({
  ...
  return (
    <GroupContainer
      className={cn("chat-system-text -mx-1 px-1", "space-y-0.5 py-0.5")}
...
function WorkGroupToggleTimelineRow({
  ...
  return (
    <button
      type="button"
      className="chat-system-text flex min-h-6 w-full cursor-pointer items-center gap-1.5 rounded-md px-0.5 py-0.5 text-left text-[12px] leading-5 transition-colors duration-150 hover:bg-accent/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring/70"
      aria-expanded={row.expanded}
      onClick={() => ctx.onToggleWorkGroup(row.groupId, row.id)}
    >
      <span className="flex size-6 shrink-0 items-center justify-center text-icon-muted">
...
function toolWorkEntryHeading(workEntry: TimelineWorkEntry): string {
  const value = normalizeCompactToolLabel(workEntry.toolTitle || workEntry.label).trim();
...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  const entryIconName =
    showWarningIndicator || (showFailedIndicator && !workEntrySignalsSevereFailure(workEntry))
      ? "x"
      : workEntryIconName(workEntry);
  const displayText =
    displayLabel ?? workEntryPreview(workEntry, workspaceRoot) ?? toolWorkEntryHeading(workEntry);
  const previewText = displayText;
  const commandMatchesVisibleLabel = workEntry.command?.trim() === previewText.trim();
  const entryToolIcon = showWarningIndicator ? undefined : workEntry.toolIcon;
  ...
  const viewedImage =
    viewedImagePath && threadRef
      ...
      : null;
  const canExpand =
    buildToolCallExpandedBody(workEntry, workspaceRoot, previewText, viewedImagePath) !== null ||
    viewedImage !== null;
