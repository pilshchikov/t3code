...
/**
 * A stacked banner plus the fork's ordering flag. Upstream's stack renders the
 * array in order; `urgent` marks the calm-styled live states that still have to
 * front it, such as an update in progress.
 */
type ComposerBannerItem = ComposerBannerStackItem & { urgent?: boolean };
import {
...
function getComposerOverlayOccupiedHeight(element: HTMLElement): number {
  return Math.ceil(Math.max(0, element.getBoundingClientRect().height));
}
...
export default function ChatView(props: ChatViewProps) {
  ...
  const systemComposerBannerItems = useMemo<ComposerBannerItem[]>(() => {
  ...
  const composerBannerItems = useMemo<ComposerBannerItem[]>(() => {
    const isUrgentSystemItem = (item: ComposerBannerItem) =>
  ...
  const getShortcutContext = useCallback(
    ...
    [previewPanelOpen, terminalUiState.terminalOpen],
  );
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    <div
                      data-with-context={showComposerContextStrip || undefined}
                      className={cn(
                        "chat-composer-glass-shell @container/composer-surface group/composer-surface relative mx-auto w-full max-w-3xl",
