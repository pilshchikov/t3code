...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const createMultiwork = useAtomCommand(multiworkEnvironment.create, {
    reportFailure: false,
  });
  const listMultiwork = useAtomCommand(multiworkEnvironment.list, {
    reportFailure: false,
  });
  ...
  const openMultiworkDialog = useCallback(
    (member: SidebarProjectGroupMember) => {
      setMultiworkTarget(member);
      setMultiworkTaskName("");
      setMultiworkError(null);
      setMultiworkCopies([]);
      void listMultiwork({ environmentId: member.environmentId, input: {} }).then((result) => {
        if (result._tag === "Success") {
          setMultiworkCopies(result.value.copies);
        }
      });
    },
    [listMultiwork],
  ...
  const registerProjectByPath = useCallback(
    async (environmentId: SidebarProjectGroupMember["environmentId"], workspaceRoot: string) => {
  ...
  const submitMultiwork = useCallback(async () => {
    ...
    setMultiworkError(null);
    try {
      const commandResult = await createMultiwork({
        environmentId: multiworkTarget.environmentId,
        input: { cwd: multiworkTarget.workspaceRoot, branch },
      });
      ...
      if (!result.reused) {
        await registerProjectByPath(multiworkTarget.environmentId, result.path);
      }
      toastManager.add(
        stackedThreadToast({
          type: "success",
  ...
  const addExistingMultiworkCopy = useCallback(
    async (copyPath: string) => {
      ...
      setMultiworkError(null);
      try {
        await registerProjectByPath(multiworkTarget.environmentId, copyPath);
        toastManager.add(
      ...
      >
        <DialogPopup className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {multiworkTarget
                ? `Create an isolated copy of ${multiworkTarget.workspaceRoot} on a fresh branch.`
