...
describe("openDiffFilePrimaryAction", () => {
  ...
  it("opens repository-relative diff files from a nested project", () => {
    ...
    expect(
      ...
      isOpen: true,
      // Fork divergence: file surfaces are keyed by workspace root as well as relative path, so
      // the same filename in two roots of a multi-directory project cannot collide.
      activeSurfaceId: `file:${encodeURIComponent("/repo/frontend")}:Dockerfile`,
    });
    expect(openInEditor).not.toHaveBeenCalled();
  });
