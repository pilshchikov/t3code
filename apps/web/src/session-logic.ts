...
export function deriveTimelineEntriesWithState(
  ...
  if (
    previous !== null &&
    (previous.turnPlans?.length ?? 0) === turnPlans.length &&
  ...
  if (canAppend) {
    ...
    const planRows = turnPlans
      .slice(previous.turnPlans?.length ?? 0)
      .map((turnPlan): TimelineEntry => ({
        id: turnPlan.id,
        kind: "turn-plan",
        createdAt: turnPlan.createdAt,
        turnPlan,
      }));
    const suffix = [...messageRows, ...proposedPlanRows, ...planRows, ...workRows].toSorted(
  ...
  const planRows = turnPlans.map((turnPlan): TimelineEntry => ({
    id: turnPlan.id,
    kind: "turn-plan",
    createdAt: turnPlan.createdAt,
    turnPlan,
  ...
  return {
    ...
    workEntries,
    turnPlans,
    entries: [...messageRows, ...proposedPlanRows, ...planRows, ...workRows].toSorted(
      compareTimelineEntriesByCreatedAt,
    ),
  };
}
