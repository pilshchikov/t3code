  ...
  lint: {
    ...
    overrides: [
      ...
      },
      {
        ...
        // has no findings and stays covered too. This fork's own panels still carry raw
        // colors from before the rule, so they report as warnings here.
        files: ["apps/web/src/**"],
      ...
      },
      {
        ...
        excludeFiles: ["apps/web/src/components/ui/**"],
        rules: {
          // This fork's own surfaces (file tree, git panels, usage, composer strip, sidebar)
          // predate these rules and restyle ui primitives in many places. They report as
          // warnings here so `vp check` errors stay meaningful, while upstream keeps them
          // as errors for its own code.
